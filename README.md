...
A small bot that checks WG-Gesucht every 15 minutes for new WG rooms in Munich priced up to 750 EUR and sends them to you on Telegram. It runs on GitHub Actions, so no computer or server needs to stay on.
...
(The bot also filters to 0-750 EUR itself, so listings outside the range are ignored either way.)
